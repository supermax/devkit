...
namespace DevKit.Serialization.Json.API
{
	...
	public sealed class JsonData : IJsonWrapper, IEquatable<JsonData>, IMapContainer, ILoggable
	{
		...
		ICollection IDictionary.Keys
		{
			...
			{
				EnsureDictionary();
				var keys = _objectList.Select(entry => entry.Key).ToList();
				return keys;
			}
		}
		...
		ICollection IDictionary.Values
		{
			...
			{
				EnsureDictionary();
				var values = _objectList.Select(entry => entry.Value).ToList();
				return values;
			}
		}
		...
		public JsonData this[string propName]
		{
			...
			set
			{
				...
				{
					Log("this[{0}] contains value already: '{1}'", propName, value);
